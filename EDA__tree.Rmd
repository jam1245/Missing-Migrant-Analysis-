...
### This analysis focuses on missing persons and deaths recorded for the primary migration routes to Europe.  

### Additional resources that help visualize and explain the data here: 
...
# We've got some leftover NAs encoded as <NA> in the data.  The normal na removal code isn't working so this bit tranforms things and changes these NA values to "unknown" in the column.  
...
# missing persons records shown on a map.  All of these appear to be around the Med. Sea. 
...
Here we plot all the missing persons and deaths by region 

```{r}
location_plot <- ggplot(missing_med, aes(x = lat, y = lon), col=incident_location) + geom_point(aes(col=incident_location))
location_plot

missing_loc_map <- ggmap(map) + geom_point(aes(x = lon, y = lat, aplpha=0.5, col=factor(incident_location)), data = missing_people)
missing_loc_map 

dead_loc_map <- ggmap(map) + geom_point(aes(x = lon, y = lat, aplpha=0.5, col=factor(incident_location)), data = dead_people)
dead_loc_map


```

Now we are going to add another feature called migration route.  This new feature is closely related to incident location.  However, we're breaking thigs down a little more based on information from Frontex and IOM defined migration routes. I think these breaks represent a good start, but they could probably be improved if you had more specialized knowledge of the routes.  
...
missing_med$route <- ifelse((missing_med$lat <= 40) & (missing_med$lat >= 30) & (missing_med$lon <= 5) & (missing_med$lon >= -20), "Western Mediterranean Route",
                      ...
                      ifelse((missing_med$incident_location == "North Africa"), "North Africa Route", "Europe"))))))
                      

ggmap(map) + geom_point(aes(x = lon, y = lat, aplpha=0.5, col=factor(route)), data = missing_med)


missing_people <- subset(missing_med, missing >= 1)
dang_route_map <- ggmap(map) + geom_point(aes(x = lon, y = lat, aplpha=0.5, col=factor(route)), data = missing_people)
dang_route_map 
...
Here we create a few more features for our data set.  We can run these through a model later to see how useful they can be.  
...
# The nationality column is pretty messey as well. We'll try a simple fix, but if we want better information this column is going to need to be cleaned.  
...
Now look at where Syrians go missing or die. 
```{r}

missing_syrians <- subset(test, nationality == 'Syrian')
syrians <- ggmap(map) + geom_point(aes(x = lon, y = lat, aplpha=0.5, col=factor(route)), data = missing_syrians)
syrians


missing_sy <- subset(missing_syrians, missing >= 1)

s <- get_map(location = 'Adana', zoom = 5)
syrians <- ggmap(s) + geom_point(aes(x = lon, y = lat, aplpha=0.5, size=missing, col='red'), data = missing_sy)
syrians


dead_sy <- subset(missing_syrians, dead >= 1)
syrians_dead <- ggmap(s) + geom_point(aes(x = lon, y = lat, aplpha=0.5, size=dead, col='red'), data = dead_sy)
syrians_dead
...
Let's now look at our drowning feature sums. 
...
We can build a basic decision tree.  
...
fit <- rpart(cause_death ~ date + route + origin, data = test, method = 'class')
